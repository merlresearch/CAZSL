...
class LatentEncoder(nn.Module):
    ...
    def forward(self, x, y):
        ...
        std = 0.1 + 0.9 * t.sigmoid(log_sigma)  ##CHANGED from soobinseo to match original
        # std = t.exp(0.5 * log_sigma) ##CHANGED from soobinseo
...
class DeterministicEncoder(nn.Module):
    ...
    def __init__(self, num_hidden, num_latent, x_dim=2, y_dim=1):
...
class Decoder(nn.Module):
    ...
    def __init__(self, num_hidden, x_dim=2, y_dim=1, boolean_pred=True):
        ...
        self.x_dim = x_dim
        self.y_dim = y_dim
        self.target_projection = Linear(self.x_dim, num_hidden)
        self.linears = nn.ModuleList([Linear(num_hidden * 3, num_hidden * 3, w_init="relu") for _ in range(3)])
        if self.boolean_pred:
            self.final_projection = Linear(num_hidden * 3, self.y_dim)
        else:
            # Predict mu, sigma
            self.final_projection = Linear(num_hidden * 3, 2 * self.y_dim)
    ...
    def forward(self, r, z, target_x):
        ...
        if self.boolean_pred:
            ...
            aux = self.final_projection(hidden)
            y_pred = aux[:, :, : self.y_dim]
            sig_pred = aux[:, :, self.y_dim :]
